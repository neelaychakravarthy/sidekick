...
- **iMessage path deferred to V2.** Photon's iMessage integration is wired code-complete (receive via webhook + send via SDK) but never went live — enabling the platform doesn't auto-provision a phone line (paid tier / sales contact gate). BlueBubbles (self-hosted macOS bridge) was investigated and reverted in increment 27: it binds to the host Mac's existing Apple ID so sends would appear from the host (wrong identity for a bot), and a separate Apple ID still needs a phone line for proper group + cross-platform UX. Twilio SMS is 1:1 (group MMS is US-only + carrier-flaky); WhatsApp Business sandbox requires recipient opt-in (friction). **Telegram is the canonical demo channel for MVP**; iMessage/SMS support is V2 work pending either a Photon line provisioning, a WhatsApp Business verification, or a paid iMessage provider (Sendblue, Loopage). Photon code paths remain in the repo as reviewable scaffolding.
