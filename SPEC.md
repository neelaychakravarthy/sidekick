...
- **Photon Spectrum** — secondary sponsor; cross-platform messaging SDK adding iMessage parity alongside Telegram.
  ...
  - **Demo blocker:** awaiting a Photon line (phone number) to be provisioned in the Photon dashboard. Enabling the platform alone doesn't auto-provision a line — gated on a paid tier or sales contact. Until a line lands, iMessage flow is inert at runtime; code is reviewable + deployable. BlueBubbles added in increment 27 as a parallel iMessage backend (self-hosted macOS bridge — see `lib/bluebubbles/`). Photon code retained as fallback; per-group backend determined by which of `groups.photon_space_id` or `groups.bluebubbles_chat_guid` is populated.
...
- **iMessage backend choice.** Two backends wired: Photon (paid, line-gated, currently inert) and BlueBubbles (free, self-hosted Mac bridge, active). Groups created via either webhook route to that backend for the lifetime of the row. No automatic failover between backends if one is unreachable.
